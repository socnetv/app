...
### Post-Phase-1 fix — canvas-clear performance (#260)

Found later, outside the Group A/B/C/Final-Gate plan above: clearing a large network (File → New,
or loading a new file over one already displayed) was slow — confirmed 30s to over a minute on
2000-node/40,000-edge networks. Two distinct causes, both fixed:
...
- **`scene()->clear()` destructor cascade (WS10 territory).** `GraphicsNode::~GraphicsNode()`
  ...
  `QGraphicsScene::clear()` already tears down every remaining item itself. Real, but — per live
  profiling below — not the dominant cost.
- **MainWindow signal re-entrancy (not WS10 territory, noted here only for the full picture).**
  Live profiling (`sample` against the actual process, driven reproducibly via the new
  ...
  `blockSignals()` guard — it re-entered `slotEditEdgeMode()`, which triggered
  `optionsEdgeArrowsAct->trigger()` and forced a synchronous full-canvas repaint via
  `statusMessage()`, while the old (still fully rendered) network was still on screen. Fixed in
  `mainwindow.cpp`, not `graphicswidget.cpp` — see #260 for the full write-up.
