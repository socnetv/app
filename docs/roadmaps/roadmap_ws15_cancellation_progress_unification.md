...
🚧 In progress. P1 ✅ done and live-verified (including the `ludcmp()` extension found necessary
during that verification — see below). P2 and P3 not started. Split off from WS5 (A5's live
...
2. **A live, reproducible crash**: graph mutation (e.g. "New") racing an in-flight background
   computation, exposed by the same threading model (P3 below) — found while testing (1), confirmed
...
3. **Two redundant progress-dialog systems coexist**, one of them silently leaving ~20 operations
   both unresponsive to Cancel *and* still blocking the GUI thread outright (P2 below) — found while
...
concurrent "New" (P3's repro) put the app into the live-race state before the process was killed
...
### P2 — Retire the linear progress-dialog system
...
**Sequencing:** Group A first (pure deletion, no dependency on P1/P3). Group B needs each site
migrated individually, golden/benchmark-verified per site, same discipline as any other
`runGraphOperationAsync` migration. Once Group B is done, the linear system
...
### P3 — Global "graph busy" guard (crash fix)
...
`ludcmp()` to start, clicked Cancel (no effect — `ludcmp()` has no cancellation check, see P1's
scope limits), then pressed Ctrl+N. Canvas went empty (`Graph::clear()` completed on the GUI thread)
**while the console kept printing `ludcmp()` log lines** — `graphThread` was still actively
...
`QtConcurrent` parallel path — any long-running `graphThread` computation is exposed.
...
**Proposed fix — leading candidate, not yet scoped in implementation detail:** a **global** busy
guard rather than a per-action one. Disable the entire menu bar and toolbar (or otherwise block all
UI interaction) for the duration of any `runGraphOperationAsync`-dispatched operation, re-enabling in
`onComplete()`. Single point of control — doesn't require correctly identifying every current and
future dangerous graph-mutating action individually (New is confirmed dangerous; Open/Close/Add-Node/
Add-Edge/etc. are presumably the same risk but not yet each individually checked), and doesn't
depend on first understanding why the modal dialog's own blocking has a gap.

**Not yet decided:**
- Disable `menuBar()`/`toolBar()` directly vs. a dedicated `MainWindow`-level busy flag checked more
  surgically at each dangerous action.
- UX: does a disabled menu/toolbar read as "busy" or as "frozen/broken"?
- Should it extend to canvas interaction (node/edge add via mouse), not just menu/toolbar actions?
- Per-window implications if multi-window is ever a thing (not currently).
...
- P2's Group A deletions and Group B migrations are each independently golden/benchmark-verified,
  one call site (or small batch) at a time — not one large diff.
- P3 needs explicit design sign-off on the open questions above before implementation starts, per
  the project's plan-before-code rule — this is a UX-visible change, not a pure internal fix.
- No GitHub issue for any of this (unreleased 3.7-cycle behavior) — fix directly.
- Once P1-P3 land: delete `CANCELLATION_FIX_NOTES.md` (superseded by this doc), add a changelog
  entry, update WS5's A5 section and WS7's status line to point here instead of duplicating content.
