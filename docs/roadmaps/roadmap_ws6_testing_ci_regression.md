...
6. ~~Add the disconnected-graph coverage axis~~ — done 2026-09-24 for `connectivity` (pre-existing),
   `matrix` (pre-existing), `vertex_connectivity` (pre-existing), `reachability`, `walks`,
   `clustering`, `distance`, `signed`. `prominence` still open — see below.

**Third coverage axis: isolates / disconnection.** Directed/undirected and weighted/unweighted are
covered per-kernel above. Isolated vertices (degree 0) and disconnected non-trivial components
are two distinct states, both now covered for every kernel except `prominence` — this matters
because three of this session's bugs (#287, #288, #290) only manifested, or manifested worse, on
graphs with unreachable pairs (the `RAND_MAX` sentinel path), and wiring the new disconnection
fixtures into `distance` surfaced three more (#292, #293, #294).

Remaining work: `prominence` has no dedicated disconnection fixture (only the pre-existing
`TinyWeightedIsolate_Undir_N4_E2`, which covers a single isolate but not a multi-component
disconnected graph). Add one (reuse `TinyDisconnectedWeighted_Undir_N6_E4`/
`TinyDisconnectedWeighted_Dir_N5_E3` from the `distance` kernel's coverage rather than a new
dataset) and independently verify before dumping, same bar as every other entry in this table.
